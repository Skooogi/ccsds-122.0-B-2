...
//static uint64_t b_mask = 0x7ffffbffffdffffe;
static uint64_t b_mask2 =  0b111111111111111111111111111111111111111111111111111111111111000ULL;
static uint64_t c_mask =  0b000000000000000000000000000000000000000000000000000000001111000ULL;
static uint64_t g_mask2 = 0b000000000000000000000000000000001111111111111111000000000000000ULL;
static uint64_t h_mask2 = 0b000000000000000000000000000000000000000000001111000000000000000ULL;
...
void block_set_status_with2(Block* block, uint64_t high_status_bit, uint64_t low_status_bit) {
    block->high_status_bit2 = high_status_bit;
    block->low_status_bit2 = low_status_bit;
}

//Transforms the status bits back to a value.
int8_t block_get_status2(Block* block, uint8_t ac_index) {
    return state_map[((block->high_status_bit2 >> ac_index) & 1) * 2 + ((block->low_status_bit2 >> ac_index) & 1)];
}
...
uint8_t block_get_bmax2(Block* block) {
    uint64_t filtered = (~block->high_status_bit2 & block->low_status_bit2) & b_mask2;
    return (filtered > 0);
}

//Status of descendants of a single family. Descendants = Children + Grandchildren.
uint8_t block_get_dmax2(Block* block, uint8_t family) {
    uint64_t filtered = (~block->high_status_bit2 & block->low_status_bit2);
    return (filtered & ( (c_mask<<(4*family)) | ((g_mask2)<<16*family)) ) > 0;
}

//Status of the grandchildren of a single family.
uint8_t block_get_gmax2(Block* block, uint8_t family) {
    uint64_t filtered = (~block->high_status_bit2 & block->low_status_bit2);
    return (filtered & (g_mask2 << (16*family))) > 0;
}

//Status of one quadrant of grandchildren of a single family.
uint8_t block_get_hmax2(Block* block, uint8_t family, uint8_t quadrant) {
    uint64_t filtered = (~block->high_status_bit2 & block->low_status_bit2);
    return (filtered & (h_mask2 << (16*family + quadrant*4))) > 0;
}
