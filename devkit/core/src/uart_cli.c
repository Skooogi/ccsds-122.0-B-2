#include "uart_cli.h"
#include "atmel_start.h"
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>

static uint8_t const crc8_table[] = {
    
};

static char error_msg[19] = "Not a valid command";

typedef struct Packet {
    uint8_t length;
    uint8_t* data;
} Packet;

static void load_file();
static void send_file();
static uint8_t crc8(Packet* packet);
static Packet* receive_packet();

static void print(const char* str) {
    io_write(&EDBG_COM.io, (uint8_t*)str, strlen(str));
}

extern volatile uint32_t data_arrived;
static volatile uint8_t packet_length = 0;
static uint8_t packet_data[256];
static void read_message() {

    packet_length = 0;
    uint8_t received = 0;
    memset(&packet_data, 0, sizeof(packet_data));

	while (1) {
		if (data_arrived == 0) {
			continue;
		}

		while (io_read(&EDBG_COM.io, &packet_data[received], 1) == 1) {
            received++;
		}
		data_arrived = 0;

        if(packet_length == 0) {
            packet_length = packet_data[0];
        }

        if(received == packet_length) {
            return;
        }
	}
}

void parse_input(uint8_t* data, uint8_t length) {

    char message_cache[256] = {};
    memcpy(&message_cache, data, sizeof(message_cache));
    char* token = strtok((char*)data, " ");

    while(token) {

        if(strcmp(token, "upload") == 0) {
            print("Downloading file!");
            Packet* packet = receive_packet();
            if(!packet) {
                print("failed");
            }
        }

        else if(strcmp(token, "download") == 0) {
            print("Sending file!");
        }

        else if(strcmp(token, "compress") == 0) {
            print("Compressing!");
        }

        else {
            print(error_msg);
        }
        token = strtok(NULL, " ");
    }
}


static Packet* receive_packet() {

    Packet* packet = malloc(sizeof(Packet));
    if(!packet) {
        print("Failed to allocate packet!");
        return NULL;
    }

    read_message();
    //Get packet size
	packet->length = packet_data[0];

    packet->data = malloc(packet->length);
    if(!packet->data || packet->length == 0) {
        print("Failed to allocate packet!");
        free(packet);
        return NULL;
    }
    memcpy(&packet->data[0], &packet_data[1], packet->length);

    uint8_t crc = 0;
    for(size_t i = 0; i < packet->length; ++i) {
        crc = crc8_table[crc ^ packet->data[i]];
    }

    if(crc) {
        free(packet->data);
        free(packet);
        return NULL;
    }

    return packet;
}

static void load_file() {

}
